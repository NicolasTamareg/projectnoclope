...
const PaymentForm = {
  ...
  computed: {
    ...
    formIsValid: function () {
      if (this.cardIsValid === 'border-green' && this.expiryIsValid === 'border-green' && this.cvcValid === 'border-green') {
        return true;
      }
      return false;
    }
  ...
  <div id="root">
    ...
    <form v-if="shouldDisplayForm" @submit.prevent="handleSubmit">
      <label for="card">
        ...
        <input @keyup.alt.enter="handleKeyUp" :class="cardIsValid" v-model="card" type="text" id="card"/>
      </label>
      <label for="expiry">
        Date d'expiration (mois / année)
        <input @keyup.alt.enter="handleKeyUp" :class="expiryIsValid" type="month" name="expiry" id="expiry" v-model="expiry" />
      </label>
      <label for="cvc">
        CVC (3 chiffres)
        <input @keyup.alt.enter="handleKeyUp" :class="cvcValid" type="text" name="cvc" id="cvc" v-model="cvc" />
      </label>
